<?php
// ============================================================================
// termino_clave.php — CRUD para la tabla Termino Clave
// Ubicacion: pages/termino_clave.php
//
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accionPost === 'crear') {
        ...
        $datos = [
            ...
            'termino_ingles' => $_POST['termino_ingles'] ?? '',
        ];

        // Llamar a la API: POST http://localhost:8000/api/termino_clave con los datos
    ...
    if ($accionPost === 'actualizar') {
        ...
        // Llamar a la API: PUT http://localhost:8000/api/termino_clave/id/{UUID}
    ...
    if ($accionPost === 'eliminar') {
        ...
        // Llamar a la API: DELETE http://localhost:8000/api/termino_clave/id/{UUID}
    ...
    header('Location: termino_clave.php');
...
$paginaActual = 'Termino Clave';   // Resalta "Gran Area" en el sidebar
$tituloPagina = 'Termino Clave';  // Aparece en la pestana del navegador (<title>)
...
// GET http://localhost:8000/api/termino_clave
...
// Despues de esto:
//   $registro = ['id'=>'...','termino'=>'...','termino_ingles'=>'...']
...
<div class="container mt-4">
    ...
    <?php if (!$mostrarFormulario): ?>
        <!-- Este link lleva a termino_clave.php?accion=nuevo, que activa el formulario vacio -->
        <!-- btn btn-primary = boton azul de Bootstrap -->
        <!-- mb-3 = margin-bottom 3 (espacio abajo) -->
        <a href="termino_clave.php?accion=nuevo" class="btn btn-primary mb-3">Nueva Termino Clave</a>
    <?php endif; ?>
        ...
        <div class="card mb-3">
            <div class="card-header">
                <!-- Operador ternario: condicion ? valor_si_true : valor_si_false
                     Si estamos editando muestra "Editar Termino Clave", si no "Nuevo Termino Clave" -->
                <?= $editando ? "Editar Termino Clave" : "Nuevo Termino Clave" ?>
            </div>
            <div class="card-body">
                <!-- method="POST" = los datos van en el body HTTP (no visibles en la URL) -->
                <!-- action="termino_clave.php" = a donde se envia el formulario (esta misma pagina) -->
                ...
                <form method="POST" action="termino_clave.php"
                      onsubmit="<?= $editando ? "return confirm('¿Está seguro de actualizar el termino de clave?')" : '' ?>">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control" name="termino"
                                value="<?= $registro['termino'] ?? '' ?>" />
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Termino en Inglés</label>
                            <input class="form-control" name="termino_ingles"
                                value="<?= $registro['termino_ingles'] ?? '' ?>" />
                        </div>
                    </div>
                    ...
                    <!-- Boton Cancelar: es un <a> (link), NO un boton de submit.
                         No envia nada, simplemente vuelve a termino_clave.php (la lista). -->
                    <!-- btn-secondary = boton gris de Bootstrap -->
                    <a href="termino_clave.php" class="btn btn-secondary">Cancelar</a>
                </form>
            </div>
        </div>
    <?php endif; ?>
    ...
    <?php if (!empty($registros)): ?>
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                <!-- foreach recorre cada registro y genera una fila <tr> por cada termino de clave.
                     $reg = un termino de clave: ['id'=>'...','termino'=>'...','termino_ingles'=>'...']
                     Se genera tanto HTML como terminos de clave haya en el array. -->
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <!-- me-1 = margin-end 1 (espacio entre este boton y el siguiente) -->
                                <a href="termino_clave.php?accion=editar&clave=<?= $reg['id'] ?>"
                        ...
                        <form method="POST" action="termino_clave.php" style="display:inline"
                              onsubmit="return confirm('¿Está seguro de eliminar el termino de clave\'<?= $reg['termino'] ?? '' ?>\'?')">
    ...
    <?php else: ?>
        ...
        <div class="alert alert-warning">No se encontraron registros en la tabla termino_clave.</div>
    <?php endif; ?>
