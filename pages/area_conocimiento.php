<?php
// ============================================================================
// area_conocimiento.php — CRUD para la tabla Area de Conocimiento
// Ubicacion: pages/area_conocimiento.php
//
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accionPost === 'crear') {
        ...
        $datos = [
            'gran_area'  => $_POST['gran_area'] ?? '',
            'area'       => $_POST['area'] ?? '',
            'disciplina' => $_POST['disciplina'] ?? ''
        ];

        // Llamar a la API: POST http://localhost:8000/api/area_conocimiento con los datos
    ...
    if ($accionPost === 'actualizar') {
        ...
        // Llamar a la API: PUT http://localhost:8000/api/area_conocimiento/id/{UUID}
    ...
    if ($accionPost === 'eliminar') {
        ...
        // Llamar a la API: DELETE http://localhost:8000/api/area_conocimiento/id/{UUID}
    ...
    header('Location: area_conocimiento.php');
...
$paginaActual = 'Area de conocimiento';   // Resalta "Gran Area" en el sidebar
$tituloPagina = 'Area de conocimiento';  // Aparece en la pestana del navegador (<title>)
...
// GET http://localhost:8000/api/area_conocimiento
...
<div class="container mt-4">
    ...
    <?php if (!$mostrarFormulario): ?>
        <!-- Este link lleva a area_conocimiento.php?accion=nuevo, que activa el formulario vacio -->
        <!-- btn btn-primary = boton azul de Bootstrap -->
        <!-- mb-3 = margin-bottom 3 (espacio abajo) -->
        <a href="area_conocimiento.php?accion=nuevo" class="btn btn-primary mb-3">Nueva Área de Conocimiento</a>
    <?php endif; ?>
        ...
        <div class="card mb-3">
            <div class="card-header">
                <!-- Operador ternario: condicion ? valor_si_true : valor_si_false
                     Si estamos editando muestra "Editar Area de conocimiento", si no "Nueva Area de conocimiento" -->
                <?= $editando ? "Editar Area de conocimiento" : "Nueva Area de conocimiento" ?>
            </div>
            <div class="card-body">
                <!-- method="POST" = los datos van en el body HTTP (no visibles en la URL) -->
                <!-- action="area_conocimiento.php" = a donde se envia el formulario (esta misma pagina) -->
                ...
                <form method="POST" action="area_conocimiento.php"
                      onsubmit="<?= $editando ? "return confirm('¿Está seguro de actualizar el area de conocimiento?')" : '' ?>">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input class="form-control" name="gran_area"
                                value="<?= $registro['gran_area'] ?? '' ?>" />
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <!-- Boton Cancelar: es un <a> (link), NO un boton de submit.
                         No envia nada, simplemente vuelve a area_conocimiento.php (la lista). -->
                    <!-- btn-secondary = boton gris de Bootstrap -->
                    <a href="area_conocimiento.php" class="btn btn-secondary">Cancelar</a>
                </form>
            </div>
        </div>
    <?php endif; ?>
    ...
    <?php if (!empty($registros)): ?>
        ...
        <table class="table table-striped table-hover">
            ...
            <tbody>
                <!-- foreach recorre cada registro y genera una fila <tr> por cada area de conocimiento.
                     $reg = un area de conocimiento: ['id'=>'...','gran_area'=>'...','area'=>'...','disciplina'=>'...']
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <!-- me-1 = margin-end 1 (espacio entre este boton y el siguiente) -->
                                <a href="area_conocimiento.php?accion=editar&clave=<?= $reg['id'] ?>"
                        ...
                        <form method="POST" action="area_conocimiento.php" style="display:inline"
                              onsubmit="return confirm('¿Está seguro de eliminar el area de conocimiento\'<?= $reg['gran_area'] ?? '' ?>\'?')">
